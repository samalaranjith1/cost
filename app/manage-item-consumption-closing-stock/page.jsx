...
export default function ManageConsumptionClosingStockPage() {
  ...
  const loadInventory = async () => {
    ...
    try {
      const [summaryData, inventoryData] = await Promise.all([
        apiRequest('GET', 'departments/consumption/closing/list', queryParams),
        apiRequest('GET', 'items/consumption/closing/history', queryParams)
      ]);
      ...
      if (inventoryData?.list) {
        setClosingStockData(inventoryData.list);
      }
    } catch (error) {
  ...
  const handleDelete = async (itemId, departmentId, dt) => {
    ...
    try {
      await apiRequest('POST', 'items/consumption/closing/delete', { dt, departmentId, itemId });
  ...
  const handleApply = async (itemId, departmentId, dt, quantity) => {
    try {
      const response = await apiRequest('POST', 'items/consumption/closing/upsert', {
        list: [{ dt, departmentId, itemId, quantity: parseFloat(quantity) }]
      });
  ...
  const exportToExcel = () => {
    if (closingStockData.length === 0) return;

    const exportData = closingStockData.map(row => ({
      'Date': row.dt,
      'Item ID': row.itemId,
      'Department': row.department?.name || '',
      'Item Name': row.item?.alias || '',
      'Closing Stock': row.quantity,
      'Item Price': row.unitPrice,
      'Cost of Closing Stock': row.totalPrice
    }));

    const ws = XLSX.utils.json_to_sheet(exportData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'KitchenStock');

    const now = new Date();
    const timestamp = now.toISOString().slice(0, 19).replace(/[-T:]/g, '');
    const fileName = `kitchen_stock_${timestamp}.xlsx`;

    XLSX.writeFile(wb, fileName);
    showToast('success', 'Success', 'Export successful!');
  };

  const totalItems = closingStockData.length;
  const totalCost = closingStockData.reduce((sum, row) => sum + (parseFloat(row.totalPrice) || 0), 0);

  return (
    <DashboardLayout title="Kitchen Closing Stock Dashboard">
          <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-t-2xl p-10 mb-8 shadow-lg">
            <h1 className="text-white text-3xl md:text-4xl font-bold mb-2">Kitchen Closing Stock Dashboard</h1>
            <p className="text-purple-50 text-lg">Track and manage your kitchen closing stock efficiently</p>
          </div>

          <div className="flex justify-end gap-4 mb-6">
            <button
              onClick={() => router.push('/add-item-consumption-closing-stock')}
              className="bg-gradient-to-br from-purple-500 to-purple-600 text-white px-6 py-3 rounded-lg font-semibold hover:from-purple-600 hover:to-purple-700 transition-all shadow-sm"
            >
              ➕ Add Closing Items
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/10 outline-none bg-white"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/10 outline-none"
                />
              </div>
              ...
              <div className="flex items-end gap-2">
                <button
                  ...
                  className="flex-1 bg-gradient-to-br from-purple-500 to-purple-600 text-white px-4 py-2.5 rounded-lg font-semibold text-sm hover:from-purple-600 hover:to-purple-700 transition-all shadow-sm"
                >
                  Fetch Data
                </button>
                <button
                ...
                >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold">{dept.itemCount || 0}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {!loading && closingStockData.length > 0 && (
            <>
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Closing Stock</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Item Price</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Cost of Closing Stock</th>
...
function ClosingStockRow({ row, onDelete, onApply }) {
    ...
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-4 py-4">
        <input
          ...
          className="w-24 px-2 py-1 border border-gray-300 rounded text-sm focus:border-purple-500 focus:ring-1 focus:ring-purple-500"
        />
      </td>
      ...
      <td className="px-4 py-4">
        <div className="flex gap-2">
          {hasChanges && (
            <button
              onClick={() => onApply(row.itemId, row.departmentId, row.dt, quantity)}
          ...
          <button
            onClick={() => onDelete(row.itemId, row.departmentId, row.dt)}
