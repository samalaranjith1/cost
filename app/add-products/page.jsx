...
export default function AddProductsPage() {  const [rows, setRows] = useState([
    { id: Date.now(), name: '', departmentId: '', masterItemId: '', price: '' }
  ]);
  ...
  const loadDropdownData = async () => {
    try {
      const [deptResponse, itemResponse] = await Promise.all([
        apiRequest('GET', 'delicia/departments'),
        apiRequest('GET', 'delicia-meta/menuitems')
      ]);
      setDepartments(deptResponse.list || []);
      setMasterItems(itemResponse.list || []);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await apiRequest('POST', 'delicia-meta/menuitem/upsert', { list: validData });

      if (response.count >= 1) {
        showToast('success', 'Success', 'Data submitted successfully!');
        setRows([{ id: Date.now(), name: '', departmentId: '', masterItemId: '', price: '' }]);
      } else {
  ...
  return (
    <DashboardLayout title="Add Menu Items">
                ...
                >
                  ...
                  <div className="flex-1">
                    <select
                      ...
                      onChange={(e) => updateRow(row.id, 'masterItemId', e.target.value)}
                      className="w-full px-4 py-2 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500"
                    >
