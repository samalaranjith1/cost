...
export default function UpdateBaseItemRecipePage() {
  ...
  const handleSubmitNew = async () => {
    ...
    try {
      const response = await apiRequest('POST', 'baseitems/ingredients/upsert', {
        baseItemId,
        list: data
      });
  ...
  const handleUpdateQuantity = async (itemId, newQuantity) => {
    try {
      await apiRequest('POST', 'baseitems/ingredients/upsert', {
        baseItemId,
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      const response = await apiRequest('POST', `baseitems/${baseItemId}/ingredients/delete`, { itemId });
  ...
  return (
    <DashboardLayout title={baseItem ? `Recipe for ${baseItem.name}` : 'Base Item Recipe'}>
      <div className="max-w-7xl mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-gray-900">{baseItem?.name || 'Loading...'}</h1>
          </div>
          ...
          <div className="bg-gray-100 rounded-2xl p-8 mb-8">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={addRows}
                  className="bg-gray-200 text-gray-800 px-6 py-2.5 rounded-lg font-semibold hover:bg-gray-300 transition-all border border-gray-400"
                >
                  + Add
                </button>
              </div>
...
function IngredientRow({ ingredient, onUpdate, onDelete, router }) {
  ...
  const itemPrice = ingredient.item?.unitPrice || ingredient.unitPrice;
